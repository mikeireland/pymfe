...
class Arm(Polyspect):
    ...
    def make_lenslets(self,fluxes=[],seeing=0.8, llet_offset=0):
        ...
        if self.mode=='std':
            yoffset = (self.lenslet_width/self.microns_pix/hex_scale*np.array([0,-s32,s32,0,-s32,s32,0])).astype(int)
            xoffset = (self.lenslet_width/self.microns_pix/hex_scale*np.array([-1,-0.5,-0.5,0,0.5,0.5,1.0])).astype(int)
        elif self.mode=='high':
            yoffset = (self.lenslet_width/self.microns_pix/hex_scale*s32*np.array([-2,2,-2,-1,-1,0,-1,-1,0,0,0,1,1,0,1,1,2,-2,2])).astype(int)
            xoffset = (self.lenslet_width/self.microns_pix/hex_scale*0.5*np.array([-2,0,2,-3,3,-4,-1,1,-2,0,2,-1,1,4,-3,3,-2,0,2])).astype(int)
        else:
        ...
        for i in range(len(fluxes)):
            ...
            the_shift = int( (llet_offset + i - self.nl/2.0)*lenslet_width/self.microns_pix )
